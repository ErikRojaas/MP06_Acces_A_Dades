...
public class PR123mainTreballadors {
    ...
    public void mostrarTreballadors() throws IOFitxerExcepcio {
        ...
        UtilsCSV.llistar(treballadorsCSV);  // Mostra el contingut
    }
    ...
    public void modificarTreballadorInteractiu() throws IOFitxerExcepcio {
        ...
        // Comprovar si l'ID existeix
        List<String> treballadorsCSV = llegirFitxerCSV();
        int linia = UtilsCSV.obtenirNumLinia(treballadorsCSV, "Id", id);
        
        if (linia == -1) {
            System.out.println("Error: Treballador amb ID " + id + " no trobat.");
            return; // Sortir si no troba l'ID
        }
    ...
    public void modificarTreballador(String id, String columna, String nouValor) throws IOFitxerExcepcio {
        ...
        UtilsCSV.actualitzarLinia(treballadorsCSV, linia, columna, nouValor);
        
        // Mostrar les dades modificades
        UtilsCSV.llistar(treballadorsCSV);
        
        // Preguntar si guardar les modificacions
        System.out.print("Voleu guardar les modificacions al fitxer? (si/no): ");
        String resposta = scanner.nextLine();

        if (resposta.equalsIgnoreCase("si")) {
            escriureFitxerCSV(treballadorsCSV);
            System.out.println("Modificacions guardades.");
        } else {
            System.out.println("Modificacions no guardades.");
        }
    }
